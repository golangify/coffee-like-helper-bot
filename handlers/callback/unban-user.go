...
func (h *CallbackHandler) unbanUser(update *tgbotapi.Update, user *models.User, args []string) {
	...
	if err = h.userService.UnbanUser(targetUser); err != nil {
		panic(err)
	}
	...
	notifMsg := tgbotapi.NewMessage(0, fmt.Sprint(
		viewuser.Text(user), " разбанил(а) ", viewuser.Text(targetUser),
	))
	notifMsg.ReplyMarkup = viewuser.InlineKeyboardEdit(targetUser)
	h.mailer.Administrator(&notifMsg)

	h.bot.Send(tgbotapi.NewMessage(targetUser.TelegramID, "Тебя разбанили."))
}
