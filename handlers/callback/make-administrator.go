...
func (h *CallbackHandler) makeAdministrator(update *tgbotapi.Update, user *models.User, args []string) {
	...
	if err = h.userService.MakeAdministrator(user, targetUser); err != nil {
		panic(err)
	}
	...
	notifMsg := tgbotapi.NewMessage(0, fmt.Sprint(
		viewuser.Text(user), " сделал(а) администратором бота ", viewuser.Text(targetUser),
	))
	notifMsg.ReplyMarkup = viewuser.InlineKeyboardEdit(targetUser)
	h.mailer.Administrator(&notifMsg)

	h.bot.Send(tgbotapi.NewMessage(targetUser.TelegramID, "Тебя назначили на роль администратора в боте!\n\nНажимай -> /help"))
}
