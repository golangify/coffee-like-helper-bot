...
func (h *CallbackHandler) addWeekdayToNotification(update *tgbotapi.Update, _ *models.User, args []string) {
	var err error
	...
	if slices.Contains(weekdays, weekday) {
		h.bot.Send(tgbotapi.NewMessage(update.FromChat().ID, fmt.Sprint(`В уведомлении "`, notification.Name, `" уже есть `, workernotificator.WeekdaysRu[weekday], ".")))
		return
	}
