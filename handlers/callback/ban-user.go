...
func (h *CallbackHandler) banUser(update *tgbotapi.Update, user *models.User, args []string) {
	...
	euser.IsBanned = true
	err = h.database.Model(&euser).UpdateColumn("is_banned", true).Error
	if err != nil {
		panic(err)
	}
	...
	notifMsg := tgbotapi.NewMessage(0, fmt.Sprint(
		viewuser.Text(user), " забанил(а) ", viewuser.Text(&euser),
	))
	notifMsg.ReplyMarkup = viewuser.InlineKeyboardEdit(&euser)
	h.mailer.Administrator(&notifMsg)

	h.bot.Send(tgbotapi.NewMessage(euser.TelegramID, "Тебя забанили."))
}
